...
public $idAlumno;
   ...
   public function actualizarDatosAjax(){
      ...
      $datos = array("Id" => $this->IdAlumno, "matricula" => $matricula,  "NoProgresivo" => $this->NoProgresivo, "CURP" => $this->CURP, "nombre" => $this->nombre, "apellidoP" => $this->apellidoP, "apellidoM" => $this->apellidoM, "fechaN" => $this->fechaN, "edad" => $this->edad,"sexo" => $this->sexo,
      ...
      $respuesta = alumnosCtr::actualizarDatosCtr($datos);
...
if(isset($_POST["Id"])){
   ...
   $a->status = $_POST["status"] == "Selecciona el status..." ? "ACTIVO" :  mb_strtoupper($_POST["status"]);
   $a->rol = $_POST["rol"] == "Selecciona el rol..." ? "ESTUDIANTE" : mb_strtoupper($_POST["rol"]);
   $a->actualizarDatosAjax();
}
